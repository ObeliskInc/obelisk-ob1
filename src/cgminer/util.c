...
static int Inet_Pton6(const char* src, char* dst)
{
    ...
    while ((ch = tolower(*src++)) != '\0') {
        ...
        if (ch == '.' && ((tp + W32NS_INADDRSZ) <= endp) && Inet_Pton4(curtok, (char*)tp) > 0) {
            tp += W32NS_INADDRSZ;
            saw_xdigit = 0;
    ...
    struct pool* pool, bool share)
{
    ...
    sprintf(user_agent_hdr, "User-Agent: %s", 'obelisk-miner/1.0.0');
...
static bool parse_notify(struct pool* pool, json_t* val)
{
    ...
    char* json_str = json_dumps(val, JSON_INDENT(1));
    ...
    snprintf(pool->ntime, NTIME_STR_SIZE + 1, "%s", ntime);
    if (pool->next_diff > 0) {
    ...
    cb1 = alloca(cb1_len);
    // TODO: MIN(cb1_len, MAX_COINBASE_SIZE)
    ...
    pool->coinbase1_len = cb1_len;

    cg_memcpy(pool->coinbase2, cb2, cb2_len);
    pool->coinbase2_len = cb2_len;
#else
    free(pool->coinbase);
...
static bool parse_diff(struct pool* pool, json_t* val)
{
    ...
    if (old_diff != diff) {
        ...
        if ((double)idiff == diff)
            applog(LOG_ERR, "++++++++ Pool %d difficulty changed to %d",
                pool->pool_no, idiff);
        else
            applog(LOG_ERR, "Pool %d difficulty changed to %.1f",
                pool->pool_no, diff);
    } else
        applog(LOG_ERR, "++++++++ ++++++++ Pool %d difficulty set to %f", pool->pool_no,
            diff);
