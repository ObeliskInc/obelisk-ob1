...
#define DECRED_HEADER_SIZE ? ? ? // bytes
#define NTIME_SIZE 8 // bytes
#define MAX_COINBASE_SIZE 128 // bytes
...
#endif
// #ifdef USE_OBELISK
extern char* opt_obelisk_options;
// #endif
...
struct pool {
    ...
    // SIA: coinbase is the "arbitrary transaction"
    unsigned char* coinbase;
    int coinbase_len;
#if (ALGO == BLAKE2B || ALGO == BLAKE256)
    unsigned char coinbase1[MAX_COINBASE_SIZE];
    int coinbase1_len;
    unsigned char coinbase2[MAX_COINBASE_SIZE];
    int coinbase2_len;
#endif
    int nonce2_offset;
...
struct work {
...
#if (ALGO == BLAKE2B)
    // The "midstate" is the final Sia header ready to pass into the M registers
    ...
    Nonce nonce; /* For devices that hash sole work */
