...
#include "klist.h"
  ...
  defined(USE_HASHFAST) || defined(USE_BITFURY) || defined(USE_BLOCKERUPTER) || defined(USE_KLONDIKE) || \
	...
	defined(USE_AVALON7) || defined(USE_OBELISK)
...
static bool io_add(struct io_data *io_data, char *buf)
{
	...
	if (tot > io_data->siz) {
		...
		if (new < tot) {
			applog(LOG_ERR, "wut? siz=%d tot=%d", tot, io_data->siz);
			new = (2 + (size_t)((float)tot / (float)SOCKBUFALLOCSIZ)) * SOCKBUFALLOCSIZ;
		}

		applog(LOG_ERR, "siz=%d tot=%d", tot, io_data->siz);
...
static void dashboard_ascstatus(struct io_data *io_data, int asc, bool isjson, bool precom)
{
	struct api_data *root = NULL;
	char *enabled;
	char *status;
	...
	if (numasc > 0 && asc >= 0 && asc < numasc) {
		...
		root = api_add_string(root, "status", status, false);
		double mhs = cgpu->total_mhashes / dev_runtime;
		root = api_add_mhs(root, "mhsAvg", &mhs, false);
		root = api_add_mhs(root, "mhsRolling", &(cgpu->rolling), false);
		root = api_add_mhs(root, "mhs1m", &cgpu->rolling1, false);
		root = api_add_mhs(root, "mhs5m", &cgpu->rolling5, false);
		root = api_add_mhs(root, "mhs15m", &cgpu->rolling15, false);
		root = api_add_int(root, "accepted", &(cgpu->accepted), false);
		root = api_add_int(root, "rejected", &(cgpu->rejected), false);
		root = api_add_int(root, "hardwareErrors", &(cgpu->hw_errors), false);
		root = api_add_mhtotal(root, "totalMhs", &(cgpu->total_mhashes), false);
		root = api_add_int64(root, "diff1Work", &(cgpu->diff1), false);
		root = api_add_diff(root, "lastShareDifficulty", &(cgpu->last_share_diff), false);
...
static void dashboard_devstatus(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	if (numasc > 0) {
		for (i = 0; i < numasc; i++) {
			dashboard_ascstatus(io_data, i, isjson, isjson && devcount > 0);
...
static void dashboard_poolstatus(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	for (i = 0; i < total_pools; i++) {
		...
		root = api_add_int(root, "pool", &i, false);
		root = api_add_escape(root, "url", pool->rpc_url, false);
		root = api_add_escape(root, "worker", pool->rpc_user, false);
		root = api_add_string(root, "status", status, false);
		root = api_add_int(root, "priority", &(pool->prio), false);
		root = api_add_int64(root, "accepted", &(pool->accepted), false);
		root = api_add_int64(root, "rejected", &(pool->rejected), false);
		...
		root = api_add_uint(root, "discarded", &(pool->discarded_work), false);
		root = api_add_uint(root, "stale", &(pool->stale_shares), false);
		root = api_add_time(root, "lastShareTime", &(pool->last_share_time), false);
		root = api_add_int64(root, "diff1Shares", &(pool->diff1), false);
		root = api_add_uint32(root, "currBlockHeight", &(pool->current_height), true);
...
static int dashboard_itemstats(struct io_data *io_data, int i, char *id, struct cgminer_stats *stats, struct cgminer_pool_stats *pool_stats, struct api_data *extra, struct cgpu_info *cgpu, bool isjson)
{
	struct api_data *root = NULL;

	if (pool_stats) {
		root = api_add_diff(root, "workDiff", &(pool_stats->last_diff), false);
		root = api_add_diff(root, "minDiff", &(pool_stats->min_diff), false);
		root = api_add_diff(root, "maxDiff", &(pool_stats->max_diff), false);
		root = api_add_uint64(root, "timesSent", &(pool_stats->times_sent), false);
		root = api_add_uint64(root, "bytesSent", &(pool_stats->bytes_sent), false);
		root = api_add_uint64(root, "timesRecv", &(pool_stats->times_received), false);
		root = api_add_uint64(root, "bytesRecv", &(pool_stats->bytes_received), false);
		root = api_add_uint64(root, "netBytesSent", &(pool_stats->net_bytes_sent), false);
		root = api_add_uint64(root, "netBytesRecv", &(pool_stats->net_bytes_received), false);
	}
...
static int driverstats(struct io_data *io_data, int i, char *id, struct cgminer_stats *stats, struct cgminer_pool_stats *pool_stats, struct api_data *extra, struct cgpu_info *cgpu, bool isjson)
{
	struct api_data *root = NULL;
...
static void dashboard_minerstats(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	for (j = 0; j < total_devices; j++) {
		...
		if (cgpu && cgpu->drv) {
			...
			sprintf(id, "%s%d", cgpu->drv->name, cgpu->device_id);
			i = dashboard_itemstats(io_data, i, id, &(cgpu->cgminer_stats), NULL, extra, cgpu, isjson);
		}
	}
	...
	for (j = 0; j < total_pools; j++) {
		...
		sprintf(id, "POOL%d", j);
		i = dashboard_itemstats(io_data, i, id, &(pool->cgminer_stats), &(pool->cgminer_pool_stats), NULL, NULL, isjson);
	}
...
#if (ALGO== BLAKE2B)
	root = api_add_const(root, "Hash Method", "Blake2b", false);
#elif (ALGO == BLAKE256)
	root = api_add_const(root, "Hash Method", "Blake256", false);
#else
...
struct CMDS {
	...
	{ "dashdevs",		dashboard_devstatus,	false,	true },
	{ "edevs",		edevstatus,	false,	true },
	{ "pools",		poolstatus,	false,	true },
	{ "dashpools",		dashboard_poolstatus,	false,	true },
	...
	{ "dashstats",		dashboard_minerstats,	false,	true },
