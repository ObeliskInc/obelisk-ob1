...
{
    char path[64];
    snprintf(path, sizeof(path), "/root/.cgminer/settings_v1.1_%s_%d.bin", name, boardID);
...
ApiError saveThermalConfig(char *name, int boardID, ControlLoopState *state)
{
    ...
    char tmppath[64];
    snprintf(path, sizeof(path), "/root/.cgminer/settings_v1.1_%s_%d.bin", name, boardID);
    snprintf(tmppath, sizeof(tmppath), "/root/.cgminer/settings_v1.1_%s_%d.bin_tmp", name, boardID);
