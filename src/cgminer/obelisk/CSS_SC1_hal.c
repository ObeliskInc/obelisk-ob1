...
#define EPOT_STARTUP_VALUE 70 // will auto-step up voltage using this as starting value of epot
#define EPOT_LOW_LIMIT 30 // exclusive limit (typical range 12 to 127)
 ...
 *  - Submit the same job to all cores.  For SC1 uses half the 64-bit bounds.
 ...
 * Assumes each example only has a single solution.
 */
int iSC1EMCTestJob(int iControlState)
{
#define EMC_NONCE_UB 0x00000007FFFFFFFF
...
#define ERR_NOBOARDS_FOUND -100
    ...
    case NEW_JOB: // update the job
        SC1GetTestJob(uiSampleJob); // Load the job structure with parameters
        // overwrite the bounds for emc test
        sSC1JobBuf.uiLBReg = 0x0000000000000000; // (was SAMPLE1_LB = 0x000000000B609366)
        sSC1JobBuf.uiUBReg = EMC_NONCE_UB; // is 2^32 -1  (was SAMPLE1_UB = 0x000000000B609386 ) // TWEAK
        (void)snprintf(caStringVar, CONSOLE_LINE_SIZE, "\r\n" MSG_TKN_INFO "Job Sample %d:\r\n", uiSampleJob + 1);
        CONSOLE_OUTPUT_IMMEDIATE(caStringVar);
        iState = LOAD_JOB;
        break; // case NEW_JOB
    ...
    case LOAD_JOB: // pre-load into the target board(s)
        ...
        for (ixI = 0; ixI < MAX_NUMBER_OF_HASH_BOARDS; ixI++) {
            ...
            if (ERR_NONE == iIsBoardValid(uiBoardUUT, true)) {
                iResult = iSetSC1OCRDividerAndBias(uiBoardUUT, controlLoopState[uiBoardUUT].currDivider, controlLoopState[uiBoardUUT].currBias);
                ...
                // sSC1XferBuf.eRegister = E_SC1_REG_OCR;
                // sSC1XferBuf.uiData = OCR_ALL_CORE_FAST_CLKENB;
                // iResult = iSC1SpiTransfer(&sSC1XferBuf);

                if (ERR_NONE != iResult) {
                    break; // for
                } else {
                    iResult = iSC1CmdPreLoadJob(&sSC1JobBuf);
                    if (ERR_NONE != iResult) {
                        break; // for
                    }
                }
    ...
    case START_JOB: // Configure the target
        ...
        for (ixI = 0; ixI < MAX_NUMBER_OF_HASH_BOARDS; ixI++) {
            ...
            if (ERR_NONE == iIsBoardValid(uiBoardUUT, true)) {
                ...
                iResult = iSC1EMCStartJob(); // submit the job
                if (ERR_NONE != iResult) {
                    break; // for
                }
            } // if (ERR_NONE == iIsBoardValid(uiBoardUUT,true))
    ...
    case READ_NONCE:
        ...
        for (ixI = 0; ixI < MAX_NUMBER_OF_HASH_BOARDS; ixI++) {
            ...
            if (ERR_NONE == iIsBoardValid(uiBoardUUT, true)) { // board must be there and no errors
                sSC1XferBuf.uiBoard = uiBoardUUT;
                iResult = iSC1EMCTestJobVerify(&uiSolution);
                //                     if (ERR_NONE == iResult) {
    ...
    if (ERR_NONE > iReturn) { // clean up if we got an error code
        (void)snprintf(caStringVar, CONSOLE_LINE_SIZE, MSG_TKN_ERROR "iSC1EMCTestJob(): Error %d in case %d\r\n", iReturn, iState);
        CONSOLE_OUTPUT_IMMEDIATE(caStringVar);
