...
ApiError ob1LoadJob(uint8_t boardNum, uint8_t chipNum, uint8_t engineNum, Job* pJob)
{
    ...
    case MODEL_DCR1: {
        ...
        for (int i = 0; i < E_DCR1_NUM_MREGS; i++) {
            error = ob1SpiWriteReg(boardNum, chipNum, engineNum, E_SC1_REG_M0 + i, &pBlake256Job->m[i]);
            if (error != SUCCESS) {
                return error;
            }
        }
...
ApiError ob1SetNonceRange(uint8_t boardNum, uint8_t chipNum, uint8_t engineNum, Nonce lowerBound, Nonce upperBound)
{
    ...
    case MODEL_SC1: {
        ...
        // TEST
        // uint64_t readLowerBound;
        // error = ob1SpiReadReg(boardNum, chipNum, engineNum, E_SC1_REG_LB, &readLowerBound);
        // if (readLowerBound != lowerBound) {
        //     applog(LOG_ERR, "lower bound MISMATCH!!!");
        // } else {
        //     applog(LOG_ERR, "lower bound GOOD!!!");
        // }
        // TEST
        return error;
    }
    case MODEL_DCR1: {
        uint32_t nonce32bit = (uint32_t)lowerBound;
        ApiError error = ob1SpiWriteReg(boardNum, chipNum, engineNum, E_DCR1_REG_LB, &nonce32bit);
        if (error == SUCCESS) {
            nonce32bit = (uint32_t)upperBound;
            error = ob1SpiWriteReg(boardNum, chipNum, engineNum, E_DCR1_REG_UB, &nonce32bit);
        }
        return error;
    }
    }
...
ApiError ob1ReadNonces(uint8_t boardNum, uint8_t chipNum, uint8_t engineNum, NonceSet* nonceSet)
{
    ...
    case MODEL_SC1: {
        ...
        int n = 0;
        // applog(LOG_ERR, "fsr=0x%016llX", fsr);
...
ApiError ob1GetDoneEngines(uint8_t boardNum, uint8_t chipNum, uint64_t* pData)
{
    ...
    case MODEL_DCR1:
        error = ob1SpiReadChipReg(boardNum, chipNum, E_DCR1_REG_EDR0, &pData[0]);
        if (error != SUCCESS) {
            return error;
        }
        error = ob1SpiReadChipReg(boardNum, chipNum, E_DCR1_REG_EDR1, &pData[1]);
    }
...
HashboardStatus ob1GetHashboardStatus(uint8_t boardNum)
{
    ...
    status.chipTemp = getAsicTemp(boardNum);
