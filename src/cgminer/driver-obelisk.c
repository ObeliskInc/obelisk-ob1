    ...
    do {
        ...
        * still refilling rather than we have no upstream work. */
        ...
        if (likely(wq->num_elems > 0)) {
            ...
            // There is no work, even after waiting a bit, so give up
            applog(LOG_ERR, "HB%u: Work queue is empty!!!!", ob->chain_id);
            return NULL;
        }
        if (sig) {
	...
	for (uint8_t chipNum = 0; chipNum < ob->staticBoardModel.chipsPerBoard; chipNum++) {
		...
		for (uint8_t engineNum = 0; engineNum < ob->staticBoardModel.enginesPerChip; engineNum++) {
			...
			for (uint8_t i = 0; i < nonceSet.count; i++) {
				...
				if (nonceResult == 2) {
					applog(LOG_ERR, "Submitting nonce=0x%08x  en2=0x%08x", nonceSet.nonces[i], ob->decredEN2[chipNum][engineNum]);
					submit_nonce(cgpu->thr[0], ob->chipWork[chipNum], nonceSet.nonces[i], ob->decredEN2[chipNum][engineNum]);
				}
			}
