...
#if (MODEL == DCR1)
int opt_ob_min_hashrate = 300;
#elif (MODEL == SC1)
int opt_ob_reboot_min_hashrate = 150;
#endif
...
#ifdef HAVE_LIBCURL
    if (strncasecmp(arg, conf_web1, sizeof(conf_web1) - 1) == 0 || strncasecmp(arg, conf_web2, sizeof(conf_web2) - 1) == 0)
...
static bool gbt_decode(struct pool* pool, json_t* res_val)
{
    ...
    if (!previousblockhash || !target || !coinbasetxn || !longpollid || !expires || !version || !curtime || !bits) {
...
static void benchfile_dspwork(struct work* work, Nonce nonce)
{
    char buf[1024];
    uint32_t dn;
    int i;
...
static bool test_work_current(struct work* work)
{
    ...
    // TODO: Determine if there is anything that we need to do in here for Sia and/or Decred
    return true;
#else
    struct pool* pool = work->pool;
...
void write_config(FILE* fcfg)
{
    ...
    for (opt = opt_config_table; opt->type != OPT_END; opt++) {
        ...
        for (p = strtok(name, "|"); p; p = strtok(NULL, "|")) {
            ...
            if (opt->type & OPT_NOARG && ((void*)opt->cb == (void*)opt_set_bool || (void*)opt->cb == (void*)opt_set_invbool) && (*(bool*)opt->u.arg == ((void*)opt->cb == (void*)opt_set_bool))) {
                fprintf(fcfg, ",\n\"%s\" : true", p + 2);
                continue;
            }

            if (opt->type & OPT_HASARG && ((void*)opt->cb_arg == (void*)opt_set_intval || (void*)opt->cb_arg == (void*)set_int_0_to_9999 || (void*)opt->cb_arg == (void*)set_int_0_to_65535 || (void*)opt->cb_arg == (void*)set_int_1_to_65535 || (void*)opt->cb_arg == (void*)set_int_0_to_5 || (void*)opt->cb_arg == (void*)set_int_0_to_10 || (void*)opt->cb_arg == (void*)set_int_1_to_10 || (void*)opt->cb_arg == (void*)set_int_24_to_32 || (void*)opt->cb_arg == (void*)set_int_0_to_100 || (void*)opt->cb_arg == (void*)set_int_0_to_255 || (void*)opt->cb_arg == (void*)set_int_1_to_255 || (void*)opt->cb_arg == (void*)set_int_0_to_7680 || (void*)opt->cb_arg == (void*)set_int_0_to_200 || (void*)opt->cb_arg == (void*)set_int_0_to_4 || (void*)opt->cb_arg == (void*)set_int_32_to_63 || (void*)opt->cb_arg == (void*)set_int_22_to_75 || (void*)opt->cb_arg == (void*)set_int_42_to_85)) {
                fprintf(fcfg, ",\n\"%s\" : \"%d\"", p + 2, *(int*)opt->u.arg);
                continue;
            }

            if (opt->type & OPT_HASARG && (((void*)opt->cb_arg == (void*)set_float_125_to_500) || (void*)opt->cb_arg == (void*)set_float_100_to_250)) {
                fprintf(fcfg, ",\n\"%s\" : \"%.1f\"", p + 2, *(float*)opt->u.arg);
                continue;
            }

            if (opt->type & (OPT_HASARG | OPT_PROCESSARG) && (opt->u.arg != &opt_set_null)) {
...
static void* stratum_sthread(void* userdata)
{
    ...
    while (42) {
        ...
        while (time(NULL) < sshare->sshare_time + 120) {
            ...
            if (opt_lowmem) {
                applog(LOG_ERR, "Lowmem option prevents resubmitting stratum share");
                break;
            }
            ...
            if (!sessionid_match) {
                applog(LOG_ERR, "No matching session id for resubmitting stratum share");
                break;
            }
        ...
        if (unlikely(!submitted)) {
            ...
            if (opt_debug || ssdiff > 0) {
                applog(LOG_ERR, "Pool %d stratum share submission lag time %d seconds",
                    pool->pool_no, ssdiff);
            }
        }
    }
...
{
#if (ALGO == BLAKE2B)
    unsigned merkle_hash[64 + 1]; // Add a binary 0x01 byte to the front when hashing
#else
    unsigned char merkle_root[32], merkle_hash[64];
#endif

#if (ALGO == BLAKE2B || ALGO == BLAKE256)
    unsigned char arb_tx[ARB_TX_SIZE];
#endif
...
static void gen_solo_work(struct pool* pool, struct work* work)
{
    ...
    for (i = 0; i < pool->merkles; i++) {
        ...
        cg_memcpy(merkle_hash + 32, merkle_bin, 32);
        gen_hash(merkle_hash, merkle_root, 64);
        cg_memcpy(merkle_hash, merkle_root, 32);
    }
...
static void rebuild_nonce(struct work* work, Nonce nonce)
{
...
bool test_nonce(struct work* work, Nonce nonce)
{
...
bool test_nonce_diff(struct work* work, Nonce nonce, double diff)
{
...
bool submit_tested_work(struct thr_info* thr, struct work* work, Nonce nonce, uint32_t extranonce2)
{
    ...
    // This was already checked in is_valid_nonce() - probably should have done it here
...
static bool new_nonce(struct thr_info* thr, Nonce nonce)
{
    struct cgpu_info* cgpu = thr->cgpu;
...
bool submit_noffset_nonce(struct thr_info* thr, struct work* work_in, Nonce nonce,
    int noffset)
{
    struct work* work = make_work();
    bool ret = false;
...
struct work* __find_work_bymidstate(struct work* que, char* midstate, size_t midstatelen, char* data, int offset, size_t datalen)
{
    ...
    {
        if (memcmp(work->midstate, midstate, midstatelen) == 0 && memcmp(work->data + offset, data, datalen) == 0) {
            ret = work;
            break;
        }
    }
...
static void wait_lpcurrent(struct pool* pool)
{
    while (!cnx_needed(pool) && (pool->enabled == POOL_DISABLED || (pool != current_pool() && pool_strategy != POOL_LOADBALANCE && pool_strategy != POOL_BALANCE))) {
        mutex_lock(&lp_lock);
...
static void* watchpool_thread(void __maybe_unused* userdata)
{
    ...
    while (42) {
        ...
        for (i = 0; i < total_pools; i++) {
            ...
            if (!pool->idle && pool_strategy == POOL_FAILOVER && pool->prio < cp_prio() && now.tv_sec - pool->tv_idle.tv_sec > opt_pool_fallback) {
...
{
    cgpu->diff_accepted = cgpu->diff_rejected = cgpu->hw_errors = 0;
}
...
bool add_cgpu(struct cgpu_info* cgpu)
{
    ...
    adjust_mostdevs();
#ifdef USE_USBUTILS
    if (cgpu->usbdev && !cgpu->unique_id && cgpu->usbdev->serial_string && strlen(cgpu->usbdev->serial_string) > 4)
...
//===========================================================================
// CODE TO PRINT BACKTRACES ON SEGMENTATION FAULTS
//===========================================================================
// #include <execinfo.h>
...
// #define EMC_TEST
    ...
    //tResult = gpio_main();
    //applog(LOG_NOTICE, "-I- GPIO Result: %d\n", tResult);
    //printf("-I- GPIO Result: %d\n", tResult);
    //NOTE: END GPIO Testing

    //NOTE: BEGIN I2C Testing
    //tResult = i2cset(2, 0x70, 1);
    //applog(LOG_NOTICE, "-I- I2C Set Result: %d\n", tResult);
    //printf("-I- I2C Set Result: %d\n", tResult);

    //tResult = i2cget(2, 0x70, I2C_SMBUS_BYTE, 0);
    //applog(LOG_NOTICE, "-I- I2C Get Result: %d\r\n", tResult);
    //printf("-I- I2C Get Result: %d\n", tResult);
    //NOTE: END I2C Testing

    //NOTE: BEGIN SPI Testing
    //tResult = spi_main(2, ptArgv);
    //applog(LOG_OBELISK, "-I- SPI  Result: %d\r\n", tResult);
    //printf("-I- SPI  Result: %d\n", tResult);
    //NOTE: END SPI Testing

    //NOTE: END TESTING SECTION
#endif
...
#else
    if (!total_devices) {
        applog(LOG_ERR, "-E- All devices disabled, cannot mine!");
