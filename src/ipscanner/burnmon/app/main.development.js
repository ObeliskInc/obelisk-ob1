...
app.on("ready", () =>
  installExtensions().then(() => {
    ...
    ipcMain.on("obidentify", (event, data) => {
      console.log("obidentify seen on IPC")
      console.log("data 2 = " + data)
      const parseData = JSON.parse(data)
      const { uiuser, uipass, host } = parseData

      const cmd = ["identify", "-u", uiuser, "-p", uipass, "-i", host]
    ...
    ipcMain.on("obupdate", (event, data) => {
      ...
      if (model === "SC1" || model === "DCR1") {
        ...
        const cmd = ["upgrade-gen1", "-u", sshuser, "-p", sshpass, "-i", host]
