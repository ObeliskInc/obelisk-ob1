...
const GEN1_LATEST_FIRMWARE_VERSION = "v1.0.0"

export interface IProps extends RouteComponentProps<any> {
  startScan(payload: BeforeScan): void
  identifyDevice(payload: IdentifyArgs): void
  spawnMDNS(): void
  ...
  render() {
    ...
    let mappedMiners = sortedMiners.map((miner, i) => {
      ...
      /*
        ...
        "PASSED",
      */
      let status = [<span>"Unknown"</span>]
      if (miner.burninStatus) {
        const numHashboards = miner.burninStatus.hashboardStatus.length

        switch (miner.burninStatus.deviceStatus) {
          case "INPROGRESS":
            // Indicate how many hashboards are being tested
            status = [
              <span>
                {`Burning in ${numHashboards} hashboard${
                  numHashboards !== 1 ? "s" : ""
                }`}
              </span>
            ]
          ...
          case "FAILED":
            status = [<span className={styles.burninFailed}>Failed: </span>]
            let hbNum = 1
            const entries: any[] = []
            for (const hbStatus of miner.burninStatus.hashboardStatus) {
              if (hbStatus.status === "FAILED") {
                entries.push(
                  <span className={styles.burninFailed}>
                    {`${hbNum > 1 ? ", " : ""}Board ${hbNum} Failed: ${
                      hbStatus.actualHashrate
                    } GHs (exp. ${hbStatus.expectedHashrate} GH/s)`}
                  </span>
                )
              } else {
                entries.push(
                  <span className={styles.burninPassed}>
                    {`${hbNum > 1 ? ", " : ""}Board ${hbNum} ${_.upperFirst(
                      _.toLower(hbStatus.status)
                    )}`}
                  </span>
                )
              }
          ...
          case "PASSED":
            status = [
              <span className={styles.burninPassed}>
                {`Passed ${numHashboards} hashboard${
                  numHashboards !== 1 ? "s" : ""
                }`}
              </span>
            ]
          ...
          default:
            status =
              _.upperFirst(_.toLower(miner.burninStatus.deviceStatus)) ||
              "Unknown"
        }
        // TODO: Hello there!
      }
    ...
    return (
      <div className={styles.container} data-tid="container">
        <div className={styles.left}>
          <h2>Obelisk Burn-In Monitor</h2>
          <span className={styles.subheading}>
            Find & Burn-In Obelisk Gen 2 Miners & Hashboards
          </span>
          <div className={styles.tabwrap}>
            ...
            <div className={styles.settings}>
              <div className={styles.infoMessage}>
                ...
                connecting over a VPN, click the Enable Manual Subnet Override
                checkbox and enter your base network address and the subnet
                mask.
              </div>
              <input
                type="checkbox"
            ...
            <table className={styles.tableHeader}>
              <thead>
                <tr>
                  ...
                  <th style={{ width: "120px", textAlign: "left" }}>MODEL</th>
                  <th style={{ textAlign: "left" }}>BURN-IN STATUS</th>
                  <th style={{ width: "90px", textAlign: "left" }}>ACTION</th>
                </tr>
              </thead>
            </table>
